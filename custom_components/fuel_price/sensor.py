...
from .const import FUEL_TABS, CONF_FUEL_TYPES, CONF_STATION, STATION_CHEAPEST
...
async def async_setup_entry(
    ...
    station: str = entry.data.get(CONF_STATION, STATION_CHEAPEST)

    async_add_entities(
        FuelPriceSensor(coordinator, fuel_key, FUEL_TABS[fuel_key][1], station)
...
class FuelPriceSensor(CoordinatorEntity[FuelPriceCoordinator], SensorEntity):
    ...
    def __init__(
        ...
        self._attr_unique_id = f"{coordinator.config_entry.unique_id}_{fuel_key}"
    ...
    def extra_state_attributes(self) -> dict:
        ...
        if self._station != STATION_CHEAPEST:
            attrs["station"] = stations[0]["station"]
            attrs["updated"] = stations[0]["updated"]
        else:
            attrs["cheapest_station"] = stations[0]["station"]
            attrs["cheapest_updated"] = stations[0]["updated"]
            attrs["stations"] = [
